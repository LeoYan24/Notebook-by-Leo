...
\begin{definition}[置信域]
    ...
    \end{enumerate}
    则称$S(\mathbf{X})$是$\boldsymbol{\theta}$的置信水平为$1-\alpha$的\textbf{置信域}(confidence region)或\textbf{置信集}，将$\inf_{\theta\in\Theta}\mathbb{P}_{\boldsymbol{\theta}}(\boldsymbol{\theta}\in S(\mathbf{X}))$称为置信系数。
\end{definition}
