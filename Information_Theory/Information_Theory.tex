...
\begin{definition}[带反馈容量]
    离散无记忆信道的带反馈容量(feedback capacity)$C_{FB}$定义为可达反馈码率的上确界，即存在$(2^{nR},n)$反馈码使得$P_e^{(n)}\to 0,n\to\infty$的最大R。
\end{definition}
...
\begin{definition}[微分熵]
    设连续随机变量$X$的概率密度函数为$f(x)$，则$X$的微分熵(differential entropy)定义为
...
\begin{definition}[联合微分熵]
    设连续随机变量$(X_1,\cdots,X_n)$的联合概率密度函数为$f(x_1,\cdots,x_n)$，则$(X_1,\cdots,X_n)$的联合微分熵定义为
    \[h(X_1,\cdots,X_n)=-\int f(x^n)\log f(x^n)dx^n=\mathbb{E} \log \frac{1}{f(X^n)}\]
\end{definition}

\begin{definition}[条件微分熵]
    设连续随机变量$(X,Y)$的联合概率密度函数为$f(x,y)$，则$X$在已知$Y$的条件下的条件微分熵定义为
...
\begin{definition}[相对熵]
    两个概率密度函数$f(x)$和$g(x)$的相对熵(relative entropy)定义为
\begin{align*}
...
\begin{definition}[互信息]
    设连续随机变量$(X,Y)$的联合概率密度函数为$f(x,y)$，则$X$与$Y$的互信息定义为
    \begin{align*}
...
\begin{definition}[典型集]
    $\forall \epsilon>0,n\in\mathbb{N}^+$，定义连续随机变量$X\sim f(x)$的典型集为
